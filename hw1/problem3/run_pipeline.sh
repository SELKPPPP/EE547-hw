...
ELAPSED=0
ANALYZER_ID=$(docker compose ps -q analyzer) 

while [ $ELAPSED -lt $MAX_WAIT ]; do
    if docker cp ${ANALYZER_ID}:/shared/analysis/final_report.json /tmp/final_report.json >/dev/null 2>&1; then
        echo "Pipeline complete"
        mkdir -p output
        mv /tmp/final_report.json output/final_report.json
        docker cp ${ANALYZER_ID}:/shared/status output/ -a >/dev/null 2>&1 || true
        break
    fi

    STATE=$(docker inspect -f '{{.State.Status}}' ${ANALYZER_ID} 2>/dev/null || echo unknown)
    if [ "$STATE" = "exited" ]; then
        echo "Analyzer exited but final_report.json not found. Logs:"
